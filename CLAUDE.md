...
## Representation authority (P3B, R7 PASS_WITH_ONE_REMEDIATION — remediation implemented, R7 closeout pending; details `docs/verification/p3b/P3B_REPRESENTATION_AUTHORITY.md` §21)
...
- P3B (Mandate, MandateVersion, MandateCoverage, CoverageSigner, AuthorityEvent) is implemented on its branch (no migration); R7 result **PASS_WITH_ONE_REMEDIATION** (operator, 2026-09-24: functionally accepted). The one remediation — SourceReference instant storability, shared with P3B — is implemented on the branch (`ee31fa3`, no migration); wait for the R7 closeout (no merge, no P4A). Next, each needing an explicit approved mission: Cases and all case-specific functions (proposed first slice P4A — Case core and case authority selection, gate R8; not started).
