...
- **Two counters.** `rowVersion` (the ETag) moves with every change. `contextRevision` moves with every change to what the case relies on — route, canonical id, source links and their states, authority selection, intake label, owner hint, packet source and Drive folder — but not with the notes. The UI shows both. *(Reconciled at R11, §24: this line first said that the intake label does not move `contextRevision`; the implementation and its test have always moved it.)*
...
| Context revision | Moves with route, canonical id, links and their states, selection, intake label, owner hint, packet source and Drive folder; not with the notes (reconciled at R11, §24) | It signals a change of what the case relies on |
...
## 24. R11 documentation reconciliation — the intake label is case context (2026-09-26, home PC)

Mission TB_R11_CLOSEOUT_MERGE_AND_P4E_PROMPT_SNAPSHOT_TO_R12 (R11 hygiene item 1), recorded on `feature/p4d-production-context` in the R11 closeout. Documentation only: no code, test, contract or schema changes.

- **Finding (raised at R11 as P4D observation 1, `P4D_PRODUCTION_CONTEXT.md` §26).** §3 said that `contextRevision` moves "but not with the intake label or notes", the §17 row said "not with the label or notes", and `CLAUDE.md` said "— not the label or notes."
- **Accepted behaviour (unchanged).** `apps/api/src/modules/cases/cases.service.ts` defines `MATERIAL_PATCH_FIELDS = ['intakeLabel', 'ownerHintId', 'packetSourceId', 'driveFolderUrl']`. A PATCH that changes the intake label moves the case's `rowVersion` and its `contextRevision` once. A notes-only PATCH moves only `rowVersion`, and a PATCH that changes nothing writes nothing. The file has a single commit, the first P4A commit `2cae729` (2026-09-24), with this line unchanged. The accepted test "patch: only the contracted fields; context fields move contextRevision, notes do not; a no-op writes nothing" (`tests/db/p4a-http.test.ts`) has asserted `contextRevision: 2` after a rename since that commit. The implementation was already what it is now when R8 accepted P4A.
- **Correction.** §3 ("Two counters") and the §17 row now name the intake label among what moves `contextRevision`; `CLAUDE.md` does the same. Each changed line is marked, and the original wording is quoted above. This reconciles the documentation and changes no behaviour: notes-only edits remain non-material, and the intake label is material under the accepted implementation. P4D reads the same behaviour, so a rename changes the production-context digest through `contextRevision`.
- **Search.** Every non-frozen file that mentions the context revision was checked (`contextRevision` / "context revision" outside `docs/reference/**`). Only these three statements said that the label does not move it. Evidence files that say a notes-only edit leaves the context revision unchanged are accurate and stay as recorded. The case page's hint (`apps/web/src/app/cases/cases.tsx`) lists what moves the context revision without naming the intake label. That hint is incomplete, not contradictory, and it is product copy: this documentation-only closeout does not change it (`P4D_PRODUCTION_CONTEXT.md` §29.4).
