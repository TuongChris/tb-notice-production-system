...
Observed project engineering state only. Not legal, canonical or case evidence; it does not supersede Drive records, the specifications or the frozen references. Updated at P2 review gate R5 (2026-09-24 UTC, first PC).
...
| Deployment | none |
| Latest CI-verified code commit | P2: `cbfd640a87d7d8ad863c8cb52931340e804166fe` (run `35948987435`, both jobs success, incl. the compiled `smoke:directory` round trip). P1.1: `8fe96ae` (run `35894704102`). P1: `5507726` (run `35885393358`). P0: `0dfc7a5` (run `35863416044`) |
| Initial migration | `20260923103912_initial_schema`, sha256 `b54c36fdaada7bdd31e08558a93d70e9e40fd4c48348681c9c2a124503426515` — still the only migration (P1, P1.1 and P2 made no schema change) |
...
| Scope | Status |
|---|---|
| P2_FIRST_PC | PASS (automated) |
| P2_CI | PASS for code commit `cbfd640` (run 35948987435); the final branch run is reported at R5 |
| P2_BROWSER | PASS 14/14 — Playwright MCP isolated browser against the compiled API on the disposable `tb_notice_test` (`yarn ui:sandbox`); supplemental to the test suites |
| R5 review | pending |
...
2. P2: operator review at R5. P3 (Route, Mandate, Case, SourceReference authoring and later) needs explicit approval.
