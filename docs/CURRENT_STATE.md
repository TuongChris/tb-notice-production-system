...
Observed project engineering state only. Not legal, canonical or case evidence; it does not supersede Drive records, the specifications or the frozen references. Updated at the R5 closeout (2026-09-24 UTC, home PC = the first PC of the P0–P2 records).

| Item | State |
|---|---|
| Branches | `bootstrap/p0-local` — the P0 branch at the P0 reproduction baseline; it takes **no P1 application code**. `feature/p1-auth-shell` — P1/P1.1, accepted at R4.1, head `8fe96ae`, unchanged since. `feature/p2-directory` — P2 directory work and its R5 closeout (this state), branched from the accepted P1.1 head `8fe96aea5a30bb01fa24c987e30689f4e6532164`. `main` untouched at `1d4103d` |
...
| Current mission | TB_R5_CLOSEOUT_AND_SINGLE_PC_DEVELOPMENT_BASELINE on `feature/p2-directory` — R5 identity-lock remediation, single-PC baseline record, P3 split; stops before P3A. Its final branch CI result is reported with the closeout |
...
| Not started | P3A and P3B (see *Next phases*), Cases and every later phase — each needs explicit approval |
...
| Deployment | none |
| Latest CI-verified code commit | P2 R5 remediation: `4b67fff631fa8b51a243fbecd64d43f76ce79d97` (run `35953912083`, both jobs success, incl. the compiled `smoke:directory` round trip). P2 at R5: `cbfd640` (run `35948987435`). P1.1: `8fe96ae` (run `35894704102`). P1: `5507726` (run `35885393358`). P0: `0dfc7a5` (run `35863416044`) |
| Initial migration | `20260923103912_initial_schema`, sha256 `b54c36fdaada7bdd31e08558a93d70e9e40fd4c48348681c9c2a124503426515` — still the only migration (P1, P1.1, P2 and the R5 remediation made no schema change) |
| Contract baseline | Wire compatibility `TB-SCHEMA-API-v1.0.0`; PFC wire id `PFC-YT-EMAIL-v1.1`; active Zod source per ADR-0002 (ACCEPTED); generated artefacts unchanged since P0 (`contracts:check` OK). P2 implements 36 of the 40 directory operations; the 4 `bindCanonical*` operations are deferred to the Source phase (decision D2) |
...
| Toolchain | Node 24.21.0, Yarn 4.18.0, TypeScript 7.0.2, Prisma 7.10.0, Zod 4.6.5, ajv 8.20.0, ajv-formats 3.0.1; P1 adds argon2 0.45.1, react-router 8.4.0, @nestjs/testing 12.0.4, happy-dom 20.14.5 (see `CLAUDE.md`); P2 adds no dependency (lockfile unchanged) |
...
| Decisions | ADR-0001 manual MySQL migration semantics (ACCEPTED); ADR-0002 Zod-first contract authoring (ACCEPTED); ADR-0003 single-PC development baseline (ACCEPTED, operator, 2026-09-24); P1 interpretations listed in `P1_AUTH_SHELL.md` §4 (accepted at R4); P2 interpretations in `P2_DIRECTORY.md` §12 (decided at R5) |
...
| R5 review | PASS_WITH_ONE_REMEDIATION (operator, 2026-09-24) |
| R5 remediation (empty identity fields locked) | IMPLEMENTED and verified; operator confirmation of the closeout pending |

## Next phases (recorded at R5; not started)

| Phase | Scope | Gate |
|---|---|---|
| **P3A — Sources + canonical bindings + Route** | SourceReference registry (4 contracted operations: `listSources`, `createSource`, `getSource`, `reviseSource`); the 4 deferred canonical-binding operations (`bindCanonicalAgency`, `bindCanonicalOwner`, `bindCanonicalLegalSubject`, `bindCanonicalSigner`); Route (9 contracted operations); the representation/navigation UI those need. Excludes Mandate, MandateVersion, MandateCoverage, CoverageSigner and AuthorityEvent | R6 |
| **P3B — Representation authority** | Mandate, MandateVersion, MandateCoverage, CoverageSigner, AuthorityEvent. Never starts automatically after R6 | R7 |
...
2. P2: operator confirmation of the R5 closeout. P3A needs an explicit mission; P3B needs its own after R6.
