...
| Current mission | TB_R7_POST_MERGE_CLOSEOUT_AND_P4A_BRANCH — post-merge reconciliation, R7 closeout records and the P4A branch `feature/p4a-case-core`; no P4A implementation. Its final branch CI result is reported with the closeout |
...
| Not started | P4A implementation (its branch `feature/p4a-case-core` is prepared; see *Next phases*), the later case phases and every later phase — each needs explicit approval |
...
| Toolchain | Node 24.21.0, Yarn 4.18.0, TypeScript 7.0.2, Prisma 7.10.0, Zod 4.6.5, ajv 8.20.0, ajv-formats 3.0.1; P1 adds argon2 0.45.1, react-router 8.4.0, @nestjs/testing 12.0.4, happy-dom 20.14.5 (see `CLAUDE.md`); P2, P3A, P3B and the R7 remediation add no dependency (lockfile unchanged) |
...
| Phase | Scope | Gate |
|---|---|---|
| **P4A — Case Core and Case Authority Selection** | Expected contracted operations (16; the P4A mission verifies them against the current contract before implementation — `P3B_REPRESENTATION_AUTHORITY.md` §22.4): Case 10 — `listCases`, `createCase`, `getCase`, `patchCase`, `deleteUnusedCase`, `ArchiveCase`, `RestoreCase`, `WorkflowCase`, `RouteBindingCase`, `CanonicalBindingCase`; CaseSource 4 — `listCaseSources`, `linkCaseSource`, `getCaseSource`, `setCaseSourceLinkState`; CaseAuthoritySelection 2 — `selectCaseAuthority`, `listCaseAuthoritySelections`. Excludes ReportedItem, CaseWork, UseMapping, CaseFact, correspondence, prompt production, candidates, validation, assessments, G1–G7, READY_FOR_SIGNER, signing and sending. Branch `feature/p4a-case-core` prepared from `main` `3649bef`; implementation needs its own explicit, approved mission | R8 |

**CaseAuthoritySelection (semantic note, operator, recorded for P4A):** "the authority chain selected/pinned for evaluation in this specific Case". It does not mean G1 PASS, current authority confirmed, legally valid authority adjudicated, signer eligibility confirmed, G7 or notice readiness; actual G1 evaluation is a later readiness phase.
...
2. P4A: implementation needs its own explicit mission (the branch is prepared; nothing is implemented); that mission first verifies the 16 expected operations against the current contract.
...
4. Non-blocking: `yarn lint` reports one warning since `ee31fa3` (`tests/api/source-rules.test.ts:183`, no-unsafe-optional-chaining; test code, lint exits 0) — fix with the next approved code change (`P3B_REPRESENTATION_AUTHORITY.md` §22.6).
