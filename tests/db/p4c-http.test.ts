...
describe('SECURITY / CONTRACT', () => {
  ...
  it('no correspondence update, delete, send, reply, read-marking or contact route exists; production writes, candidates and readiness stay unrouted; P4C creates no later-phase record', async () => {
    const w = await world();
    const message = await capture(w.agency.data.id);
    ...
    const paths: Array<['GET' | 'POST' | 'PATCH' | 'DELETE', string]> = [
      ...
      // The production context is a GET-only read since P4D (tests/db/p4d-http.test.ts), and
      // prompts are routed since P4E (tests/db/p4e-http.test.ts).
